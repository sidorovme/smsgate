...
8. Публикует статус и доступность (keep-alive) в MQTT, принимает команды из MQTT (см. ниже)

### MQTT-топики

Для шлюза с базовым именем `<gw>` (например `gate-1`):

| Топик | Направление | Описание |
|---|---|---|
| `sms/incoming/<gw>` | публикация | Входящее SMS в формате PDU (JSON, см. ниже) |
| `sms/status/<gw>` | публикация (retained) | JSON-метрики: uptime, RSSI, IP, кол-во SMS, heap, состояние модема. Обновляется каждые 30 сек |
| `sms/status/<gw>/availability` | публикация (retained, LWT) | `online` / `offline` — keep-alive. `offline` выставляется брокером автоматически при обрыве связи (Last Will) |
| `sms/cmd/<gw>` | подписка | Команды устройству (см. ниже) |

**Команды** (payload — обычный текст в `sms/cmd/<gw>`):

| Команда | Действие |
|---|---|
| `reboot` | Перезагрузка ESP32 |
| `reset-modem` | Power cycle SIM900: выкл → вкл питание, переинициализация |
| `status` | Немедленно опубликовать статус в `sms/status/<gw>` |

Пример: `mosquitto_pub -h <broker> -t sms/cmd/gate-1 -m reboot`
...
7. Отслеживает доступность шлюзов (`sms/status/<gw>/availability`) и шлёт в Telegram алерт при уходе в offline / возврате online
